            ...
            @if (Session::has('status'))
                ...
                <x-table>
                    ...
                    <x-slot name="body">
                        ...
                        @foreach ($insurenceCategories as  $insurenceCategory)
                        <x-table.row wire:key="row-{{$insurenceCategory->id }}">
                            ...
                            <x-table.cell>{{ $insurenceCategory->created_at->format(config('app_settings.date_format.value')) }}</x-table.cell>
                            <x-table.cell>
                               ...
                               <div class="dropdown dropup dropleft">
                                    ...
                                    <ul class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                                        {{-- @can('edit-insurence-category') --}}
                                            <li><a class="dropdown-item"  data-original-title="Edit" title="Edit" href="{{ route('edit-insurence-category', $insurenceCategory) }}">Edit</a></li>
                                        {{-- @endcan --}}
                                        <li><a class="dropdown-item text-danger"  data-original-title="Remove" title="Remove" wire:click="destroyConfirm({{ $insurenceCategory->id }})">Delete</a></li>
                                    </ul>
                                </div>
                ...
                <div id="datatable-bottom">
                    {{ $insurenceCategories->links() }}
                </div>
                @if( $insurenceCategories->total() == 0)
                    <div>
